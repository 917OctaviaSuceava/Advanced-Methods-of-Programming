...
import com.example.demo.Controller.GarbageCollector;
...
public class Controller {
    ...
    public void oneStep() throws ProgramException, Exception, InterruptedException
    {
        executor = Executors.newFixedThreadPool(2);
        removeCompletedPrg(repository.getPrgStateList());
        List<PrgState> programStates = repository.getPrgStateList();
        if(programStates.size() > 0)
        {
            try {
                oneStepForAllPrg(repository.getPrgStateList());
            ...
            programStates.forEach(e -> {
                try {
                    ...
                    e1.printStackTrace();
                    // throw new ProgramException(e1.getMessage());
                    // System.out.println();
                }
            });
            removeCompletedPrg(repository.getPrgStateList());
